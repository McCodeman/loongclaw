...
pub(super) fn execute_external_skills_search_tool_with_config(
    ...
    let payload = request
        .payload
        .as_object()
        .ok_or_else(|| "external_skills.search payload must be an object".to_owned())?;
    let query = parse_required_query(payload, "external_skills.search")?;
    let max_results = parse_optional_search_limit(payload)?
        .unwrap_or(5)
        .clamp(1, 10);
    let source_kinds = parse_external_skill_search_sources(payload)?;
...
fn parse_optional_search_limit(payload: &Map<String, Value>) -> Result<Option<usize>, String> {
    let Some(value) = payload.get("max_results") else {
        return Ok(None);
    };
    let raw_value = value.as_u64().ok_or_else(|| {
        "external_skills.search payload.max_results must be an integer".to_owned()
    })?;
    let parsed_value = usize::try_from(raw_value)
        .map_err(|error| format!("invalid external_skills.search payload.max_results: {error}"))?;
...
fn parse_external_skill_search_sources(
    ...
    let items = value
        .as_array()
        .ok_or_else(|| "external_skills.search payload.sources must be an array".to_owned())?;
    if items.is_empty() {
        return Ok(default_external_skill_search_sources());
    }
    ...
    for item in items {
        let raw_source = item.as_str().ok_or_else(|| {
            "external_skills.search payload.sources must contain only strings".to_owned()
        })?;
...
fn extract_clawhub_download_url(document: &str, landing_url: &str) -> Result<String, String> {
    ...
    let regex = CLAWHUB_DOWNLOAD_RE.get_or_init(|| {
        Regex::new(r#"href=["']([^"']*/api/v1/download[^"']*)["']"#).ok()
    });
    let Some(regex) = regex.as_ref() else {

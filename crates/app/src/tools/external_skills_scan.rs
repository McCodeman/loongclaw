...
fn append_malware_findings(
    ...
    let curl_pipe_re = CURL_PIPE_RE.get_or_init(|| {
    ...
    let destructive_delete_re = DESTRUCTIVE_DELETE_RE.get_or_init(|| {
        Regex::new(r"(?im)\b(sudo\s+)?rm\s+-rf\s+(/|\$HOME|\~)").ok()
    });
    ...
    let netcat_exec_re = NETCAT_EXEC_RE.get_or_init(|| {
        Regex::new(r"(?im)\bnc\b[^\n]{0,120}\s-e\s").ok()
    });
